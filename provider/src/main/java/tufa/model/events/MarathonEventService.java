...
import tufa.model.events.deployments.DeploymentFailedEvent;
import tufa.model.events.deployments.DeploymentInfoEvent;
import tufa.model.events.deployments.DeploymentSuccessEvent;
import tufa.model.events.status.HealthStatusChangedEvent;
import tufa.model.events.status.StatusUpdateEvent;
import tufa.model.events.status.UnhealthyTaskKillEvent;
...
public class MarathonEventService {
    ...
    public static final Logger log = Logger.getLogger(MarathonEventService.class.getName());
    ...
    public MarathonEventService(String apiURL) {
    ...
    private void parseMarathonEvent(String name, String data) {
        ...
        switch (name) {
            ...
            case "unhealthy_task_kill_event":
                ...
                break;
            default:
                log.log(Level.WARNING, "Unknown ["+ name + "] event received from Marathon with data : " + data);
        }
    ...
    private PaaSDeploymentStatusMonitorEvent parseDeploymentFailedEvent(String data) {
        ...
        try {
            ...
            log.log(Level.SEVERE, "Unable to parse event time from Marathon", e);
